...
export function normalizeGithubExpression(condition) {
...
export function conditionUsesAlways(condition) {
  return /\balways\s*\(\s*\)/.test(normalizeGithubExpression(condition));
}

export function conditionRequiresSupabaseRelevance(condition) {
  return /(?:^|[^\w.])steps\.detect_changes\.outputs\.supabase_relevant\s*==\s*['"]true['"](?:$|[^\w.])/.test(normalizeGithubExpression(condition));
}

function validateAlwaysRelevantCondition(step, label, fail) {
  if (!step || typeof step.if !== "string" || !step.if.trim()) {
    fail(`${label} condition is missing.`);
    return;
  }
  if (!conditionUsesAlways(step.if)) fail(`${label} condition must include always() so it runs after failures.`);
  if (!conditionRequiresSupabaseRelevance(step.if)) fail(`${label} condition must require steps.detect_changes.outputs.supabase_relevant == 'true'.`);
}
...
export function validateWorkflowStructure(workflow) {
  ...
  if (!workflow) fail("Workflow is missing");
    ...
    if (workflow.on?.pull_request?.paths || workflow.on?.pull_request?.["paths-ignore"]) fail("pull_request must not use paths or paths-ignore");
    ...
    if (!job) fail("validation job missing");
      ...
      for (const step of steps) {
        if (step["continue-on-error"] === true || step["continue-on-error"] === "true") fail("continue-on-error must not be used");
      }
      ...
      validateAlwaysRelevantCondition(cleanup, "Cleanup step", fail);
      const artifact = steps.find((step) => step.uses === "actions/upload-artifact@v4");
      validateAlwaysRelevantCondition(artifact, "Artifact upload", fail);
...
function main() {
  ...
  for (const error of validateWorkflowStructure(workflow)) fail(error);

  if (sha256CanonicalText(root, BASELINE_PATH) !== EXPECTED_BASELINE_SHA) fail("Baseline SHA mismatch");

  const safety = readJson("reports/supabase-ci/repository-safety-result.json", fail);
  const negative = readJson("reports/supabase-ci/ci-negative-result.json", fail);
  const evidence = readJson("reports/supabase-ci/ci-evidence-result.json", fail);
  const cleanup = existsSync(join(root, "reports/supabase-ci/ci-cleanup-result.json")) ? readJson("reports/supabase-ci/ci-cleanup-result.json", fail) : null;
